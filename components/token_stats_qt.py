...
from config.model_config import (
    MODEL_CONFIGS, DEFAULT_MODEL_ID, get_model_by_id, ModelConfig,
)
...
class TokenStatsPanelQt(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._warning_label.setStyleSheet(f"color: {ThemeColors.ERROR}; font-size: 12px;")
    ...
    def update_stats(
        ...
        context_length = self._selected_model.context_length if self._selected_model else 200000
        ...
        self._percentage_label.setStyleSheet(f"font-size: 12px; font-weight: 500; color: {color};")
        ...
        if level == "critical":
            ...
            self._warning_label.setStyleSheet(f"color: {ThemeColors.WARNING}; font-size: 12px;")
        else:
            self._warning_banner.hide()
    ...
    def _on_model_changed(self, index: int) -> None:
        ...
        if model_id:
            ...
            if model:
                ...
                # Re-render
                self.update_stats(self._file_count, self._file_tokens, self._instruction_tokens)
