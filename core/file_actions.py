    ...
    try:
        ...
        for change in action.changes:
            ...
            if result:
                ...
                change_results.append(f"Success: Applied change: \"{change.description}\"")
            else:
                change_results.append(f"Error: Search text not found: \"{normalized_search[:30]}...\"")
        ...
        if success_count > 0:
            ...
            return ActionResult(
                ...
                action="modify",
                success=True,
                message=f"Applied {success_count}/{len(action.changes)} modifications. {'; '.join(change_results)}"
            )
        else:
            return ActionResult(
                path=action.path,
