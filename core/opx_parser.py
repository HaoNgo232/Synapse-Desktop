...
class ChangeBlock:
    ...
    occurrence: Optional[Union[Literal["first", "last"], int]] = None  # Disambiguator cho modify
...
PAIRED_REGEX = re.compile(r'<\s*edit\b([^>]*)>([\s\S]*?)<\s*/\s*edit\s*>', re.IGNORECASE)
...
FIND_TAG_REGEX = re.compile(r'<\s*find\b([^>]*)>([\s\S]*?)<\s*/\s*find\s*>', re.IGNORECASE)
...
def _build_file_action(edit: ParsedEdit, display_index: int) -> tuple[Optional[FileAction], Optional[str]]:
    ...
    if not op:
        missing.append("op")
    return None, f"Edit #{display_index}: missing required attribute(s): {' '.join(missing)}. attrs=\"{attrs_str}\""
...
def _handle_create_or_replace(body: str, op: str, file_action: FileAction) -> None:
    ...
    description = _extract_why(body) or ("Create file" if op == "new" else "Replace file")
    
    file_action.changes.append(ChangeBlock(
...
def _normalize_occurrence(occurrence_raw: Optional[str]) -> Optional[Union[Literal["first", "last"], int]]:
...
def _map_op_to_action(op: str) -> Optional[Literal["create", "rewrite", "modify", "delete", "rename"]]:
