...
Port tu: /home/hao/Desktop/labs/overwrite/src/prompts/index.ts
"""
...
def _filter_selected_tree(items: list[TreeItem], selected_paths: set[str]) -> list[TreeItem]:
    ...
    for item in items:
        ...
        has_selected_children = any(
            _has_selected_descendant(child, selected_paths)
            for child in item.children
        )
        ...
        if is_selected or has_selected_children:
            ...
            filtered_item = TreeItem(
                ...
                path=item.path,
                is_dir=item.is_dir,
                children=_filter_selected_tree(item.children, selected_paths) if item.children else []
            )
