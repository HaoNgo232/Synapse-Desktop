...
class TokenBudgetManager:
    ...
    def fit_files_to_budget(
        ...
        if estimated <= self._max_tokens * 0.8:
            # Đủ budget, đọc full content
            return self._read_full_content(
        ...
        # Strategy 4: Truncate largest files
    ...
    def _try_smartify_dependencies(
        ...
        """Strategy 2: Full primary, smart deps."""
        file_contents = {}
        total_tokens = self._tok.count_tokens(instructions) + 500
        smartified = []

        # Full content for primary
    ...
    def _try_slice_primary(
        ...
        """Strategy 3: Slice primary, smart deps."""
        file_contents = {}
        total_tokens = self._tok.count_tokens(instructions) + 500
        sliced = []
        smartified = []
        ...
        for file_path in primary:
            try:
                ...
                total_tokens += self._tok.count_tokens(slice_result.content)
    ...
    def _try_truncate_largest(
        ...
        # Combine all files and sort by size
        all_files = [(p, True) for p in primary] + [(p, False) for p in deps]
        all_files.sort(key=lambda x: x[0].stat().st_size if x[0].exists() else 0)
