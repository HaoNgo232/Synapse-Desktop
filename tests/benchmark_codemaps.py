...
def benchmark_smart_parse(content: str, iterations: int = 10) -> dict:
    ...
    return {
        ...
        "overhead_percent": ((with_time / without_time) - 1) * 100 if without_time > 0 else 0,
    }
...
def benchmark_tree_reuse(content: str, iterations: int = 10) -> dict:
    ...
    return {
        ...
        "improvement_percent": ((old_way_time / new_way_time) - 1) * 100 if new_way_time > 0 else 0,
    }
...
def benchmark_boundaries_map(content: str, iterations: int = 10) -> dict:
    ...
    num_lookups = 100
    test_lines = list(range(0, len(lines), max(1, len(lines) // num_lookups)))[:num_lookups]
...
def run_benchmarks():
    ...
    print(f"    Overhead:              {results['overhead_ms']:.2f}ms ({results['overhead_percent']:.1f}%)")
