...
def context_view(qtbot, workspace_path):
    ...
    # Select files
    view.file_tree_widget.set_selected_paths({str(workspace_path / "a.py"), str(workspace_path / "b.py")})
...
def test_compress_output_with_semantic_toggle(qtbot, context_view, workspace_path):
    ...
    with patch.object(view._prompt_builder, 'build_prompt_full', wraps=view._prompt_builder.build_prompt_full) as mock_build:
        # Click Compress
...
def test_semantic_index_affects_compressed_output(qtbot, context_view, workspace_path):
    ...
    with patch.object(view._prompt_builder, 'build_prompt', wraps=view._prompt_builder.build_prompt) as mock_build:
