...
class TestBuildIgnorePatterns:
    ...
    def test_vcs_dirs_luon_co_mat(self, tmp_path: Path):
        ...
        engine = IgnoreEngine()
        patterns = engine.build_ignore_patterns(tmp_path, use_default_ignores=False)
        for vcs_dir in engine.VCS_DIRS:
    ...
    def test_default_ignores_khi_enabled(self, tmp_path: Path):
        ...
        engine = IgnoreEngine()
        patterns = engine.build_ignore_patterns(tmp_path, use_default_ignores=True)
    ...
    def test_default_ignores_khi_disabled(self, tmp_path: Path):
        ...
        engine = IgnoreEngine()
        patterns = engine.build_ignore_patterns(
            tmp_path, use_default_ignores=False, use_gitignore=False
        )
        # Chi co VCS dirs
        assert len(patterns) == len(engine.VCS_DIRS)
    ...
    def test_thu_tu_uu_tien(self, tmp_path: Path):
        ...
        for i, vcs in enumerate(engine.VCS_DIRS):
...
class TestBuildPathspec:
    ...
    def test_match_gitignore_patterns(self, tmp_path: Path):
        ...
        spec = engine.build_pathspec(
            tmp_path, use_default_ignores=False, use_gitignore=True
        )
...
class TestReadGitignore:
    ...
    def test_cache_hoat_dong(self, tmp_path: Path):
        ...
        patterns1 = engine.read_gitignore(tmp_path)
        patterns2 = engine.read_gitignore(tmp_path)
...
class TestFindGitRoot:
    ...
    def test_tim_git_root(self, tmp_path: Path):
        ...
        root = engine.find_git_root(sub_dir)
    ...
    def test_git_root_la_path_hien_tai(self, tmp_path: Path):
        ...
        root = engine.find_git_root(tmp_path)
        assert root == tmp_path
...
class TestClearCache:
    ...
    def test_clear_gitignore_cache(self, tmp_path: Path):
        ...
        # Populate cache
        engine.clear_cache()
        engine.read_gitignore(tmp_path)
    ...
    def test_clear_pathspec_cache(self, tmp_path: Path):
        ...
        engine = IgnoreEngine()
        engine.clear_cache()
        engine.build_pathspec(tmp_path, use_default_ignores=False, use_gitignore=False)
        engine.clear_cache()
