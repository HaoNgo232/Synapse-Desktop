...
from mcp_server.handlers.analysis_handler import register_tools
...
async def test_find_references_path_traversal(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "find_references")
    result = await tool(symbol_name="foo", workspace_path=str(mock_workspace / "../../../etc"))
...
async def test_get_symbols_path_traversal(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "get_symbols")
    result = await tool(file_path="../../../etc/passwd", workspace_path=str(mock_workspace))
...
@pytest.mark.asyncio
async def test_find_references_with_extensions(mcp_instance, mock_workspace, monkeypatch):
...
@pytest.mark.asyncio
async def test_find_references_exception_handling(mcp_instance, mock_workspace, monkeypatch):
