...
async def test_read_file_range_partial(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "read_file_range")
    result = await tool(relative_path="test.py", start_line=2, end_line=3, workspace_path=str(mock_workspace))
...
async def test_read_file_range_path_traversal(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "read_file_range")
    result = await tool(relative_path="../../../etc/passwd", workspace_path=str(mock_workspace))
...
async def test_read_file_range_nonexistent(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "read_file_range")
    result = await tool(relative_path="nonexistent.py", workspace_path=str(mock_workspace))
