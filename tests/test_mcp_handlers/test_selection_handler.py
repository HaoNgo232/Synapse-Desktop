...
from mcp_server.handlers.selection_handler import register_tools
...
async def test_manage_selection_set(mcp_instance, mock_workspace):
    """Test set action"""
    tool = get_tool(mcp_instance, "manage_selection")
    result = await tool(action="set", paths=["test.py"], workspace_path=str(mock_workspace))
...
async def test_manage_selection_add(mcp_instance, mock_workspace):
    ...
    result = await tool(action="add", paths=["test.py"], workspace_path=str(mock_workspace))
...
async def test_manage_selection_path_traversal(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "manage_selection")
    result = await tool(action="set", paths=["../../../etc/passwd"], workspace_path=str(mock_workspace))
...
async def test_manage_selection_nonexistent_file(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "manage_selection")
    result = await tool(action="set", paths=["nonexistent.py"], workspace_path=str(mock_workspace))
