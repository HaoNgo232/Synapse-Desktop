...
async def test_explain_architecture_invalid_focus(mcp_instance, mock_workspace):
    ...
    tool = get_tool(mcp_instance, "explain_architecture")
    result = await tool(focus_directory="../../../etc", workspace_path=str(mock_workspace))
    
    assert "Error" in result or "Invalid" in result


@pytest.mark.asyncio
async def test_get_project_structure_empty_workspace(mcp_instance, tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_get_project_structure_with_multiple_frameworks(mcp_instance, mock_workspace):
