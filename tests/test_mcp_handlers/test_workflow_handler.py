...
async def test_rp_build_basic(mcp_instance, mock_workspace):
    ...
    with patch(
        "application.workflows.context_builder.run_context_builder",
...
async def test_rp_review_basic(mcp_instance, mock_workspace):
    ...
    with patch(
        "application.workflows.code_reviewer.run_code_review", return_value=mock_result
    ):
...
async def test_rp_refactor_discover(mcp_instance, mock_workspace):
    ...
    with patch(
        "application.workflows.refactor_workflow.run_refactor_discovery",
        return_value=mock_result,
    ):
...
async def test_rp_investigate_basic(mcp_instance, mock_workspace):
    ...
    with patch(
        "application.workflows.bug_investigator.run_bug_investigation",
        return_value=mock_result,
    ):
...
async def test_rp_test_basic(mcp_instance, mock_workspace):
    ...
    with patch(
        "application.workflows.test_builder.run_test_builder", return_value=mock_result
    ):
...
async def test_rp_refactor_plan_with_discovery(mcp_instance, mock_workspace):
    ...
    with patch(
        "application.workflows.refactor_workflow.run_refactor_planning",
        return_value=mock_result,
    ):
        tool = get_tool(mcp_instance, "rp_refactor")
        result = await tool(
