...
def test_typing_api_key_shows_indicator_and_debounces(qtbot):
    ...
    with patch(
        ...
        assert (
            "Unsaved" in view._auto_save_indicator.text()
            or view._auto_save_timer.isActive()
        )
        ...
        # Đã lưu thành công
        assert (
            "Saved" in view._auto_save_indicator.text()
            or not view._auto_save_timer.isActive()
        )
