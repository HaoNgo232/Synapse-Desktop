...
def test_set_related_mode_activate(context_view):
    ...
    view = context_view
    with patch.object(view._related_controller, "_resolve_related_files"):
        view._related_controller.set_mode(True, 2)
        assert view._related_controller.is_active is True
        assert view._related_controller.depth == 2
...
def test_set_related_mode_deactivate(context_view):
    ...
    view._related_controller._last_added_related_files = set()
    view._related_controller.set_mode(False, 0)
...
def test_update_related_button_text_off(context_view):
    ...
    view._related_controller._update_button_text()
...
def test_update_related_button_text_with_depth(context_view):
    ...
    view = context_view
    view._related_controller._mode_active = True
    view._related_controller._depth = 1
    view._related_controller._last_added_related_files = set()
    view._related_controller._update_button_text()
...
def test_update_related_button_text_with_count(context_view):
    ...
    view = context_view
    view._related_controller._mode_active = True
    view._related_controller._depth = 3
    view._related_controller._last_added_related_files = {"a.py", "b.py", "c.py"}
    view._related_controller._update_button_text()
...
def test_deactivate_clears_added_files(context_view):
    ...
    view = context_view
    view._related_controller._mode_active = True
    view._related_controller._last_added_related_files = {"file1.py", "file2.py"}
    view.file_tree_widget.remove_paths_from_selection = MagicMock(return_value=2)

    view._related_controller._deactivate()
...
def test_apply_related_results_when_deactivated(context_view):
    """Kiem tra _apply_results khong lam gi khi mode da tat."""
    view = context_view
    view._related_controller._mode_active = False
    view._related_controller._apply_results({"new.py"})


def test_apply_related_results_adds_and_removes(context_view):
    """Kiem tra _apply_results them files moi va xoa files cu."""
    view = context_view
    view._related_controller._mode_active = True
    ...
    view._related_controller._apply_results({"new.py", "user.py"})

    view.file_tree_widget.remove_paths_from_selection.assert_called_once_with(
        {"old.py"}
    )
    view.file_tree_widget.add_paths_to_selection.assert_called_once_with(
        {"new.py", "user.py"}
    )
    assert view._related_controller._last_added_related_files == {"new.py", "user.py"}


def test_apply_related_results_empty(context_view):
    """Kiem tra _apply_results voi empty set -> shows No related (line 159-160)."""
    view = context_view
    view._related_controller._mode_active = True
    view._related_controller._last_added_related_files = set()
    ...
    view._related_controller._apply_results(set())

    assert view._related_controller._last_added_related_files == set()
...
def test_depth_name_mapping(context_view):
    ...
    for depth, name in depth_names.items():
        view._related_controller._depth = depth
        view._related_controller._update_button_text()
...
def test_depth_name_unknown(context_view):
    ...
    view._related_controller._mode_active = True
    view._related_controller._last_added_related_files = set()
    view._related_controller._depth = 10
    view._related_controller._update_button_text()
...
def test_resolve_related_files_no_source_files(mock_schedule, context_view):
    ...
    view._related_controller._last_added_related_files = {"old_related.py"}
    view.file_tree_widget.remove_paths_from_selection = MagicMock()
