...
class ApplyView:
    ...
    def build(self) -> ft.Container:
        ...
        return ft.Container(
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text("OPX Response", weight=ft.FontWeight.W_600, size=14, color=ThemeColors.TEXT_PRIMARY),
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text("Results", weight=ft.FontWeight.W_600, size=14, color=ThemeColors.TEXT_PRIMARY),
    ...
    def _create_result_row(
        ...
        # Status icon
        if is_preview:
            status_icon = ft.Icon(ft.Icons.VISIBILITY, size=16, color=ThemeColors.TEXT_MUTED)
        elif success:
            status_icon = ft.Icon(ft.Icons.CHECK_CIRCLE, size=16, color=ThemeColors.SUCCESS)
        else:
            status_icon = ft.Icon(ft.Icons.ERROR, size=16, color=ThemeColors.ERROR)
        
        # Background color for row
        row_bg = ThemeColors.BG_ELEVATED if success else "#FEF2F2"  # Light red for errors
        
        return ft.Container(
            content=ft.Row([
                status_icon,
                ft.Container(
                    content=ft.Text(action, size=11, weight=ft.FontWeight.W_600, color="#FFFFFF"),
                    bgcolor=badge_color,
                    padding=ft.padding.symmetric(horizontal=8, vertical=3),
                    border_radius=4
                ),
                ft.Text(path, size=12, weight=ft.FontWeight.W_500, color=ThemeColors.TEXT_PRIMARY, expand=True),
                ft.Text(
