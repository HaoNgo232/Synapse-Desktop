...
class HistoryViewQt(QWidget):
    ...
    def _create_entry_widget(self, entry: HistoryEntry) -> QWidget:
        """Tao widget cho mot entry item trong list.

        Layout 3 dong:
        Dong 1: [dot] time  file_count  [status_badge]
        Dong 2: Tom tat operation types (3 MODIFY / 1 CREATE)
        ...
        # Sap xep theo so luong giam dan, lay 3 loai dau
        op_parts = [
            f"{count} {op}"
        ...
        # ===================== DONG 1 =====================
        # [dot 6px] time  file_count       [status badge]
        line1 = QHBoxLayout()
        line1.setSpacing(6)
        ...
        # Status badge/pill (noi bat hon text thuan)
        ...
        # ===================== DONG 2 =====================
        # Operation summary: "3 MODIFY / 1 CREATE"
        if op_summary:
            line2 = QHBoxLayout()
            line2.setContentsMargins(14, 0, 0, 0)  # Indent sau dot
            line2.setSpacing(0)
        ...
        # ===================== DONG 3 =====================
        # done X / fail Y (ro rang hon +/- de nham voi git)
        line3 = QHBoxLayout()
        ...
        fail_label.setStyleSheet(
            f"""
            color: {ThemeColors.ERROR if entry.fail_count > 0 else ThemeColors.TEXT_MUTED};
            font-size: 11px;
            font-family: 'JetBrains Mono', monospace;
        """
        )
    ...
    def _create_files_section(self, entry: HistoryEntry) -> QWidget:
        ...
        # Container an cho files con lai (an mac dinh)
        ...
        if total_actions > display_limit:
            ...
            def _toggle_files(btn=more_btn, container=hidden_container, rem=remaining):
                ...
                if is_visible:
                    btn.setText(f"Show {rem} more files")
                else:
    ...
    def _create_errors_section(self, entry: HistoryEntry) -> QWidget:
        ...
        # Container an cho errors con lai
        if total_errors > display_limit:
            ...
            def _toggle_errors(btn=more_btn, container=hidden_container, rem=remaining):
                ...
                container.setVisible(not is_visible)
                if is_visible:
                    btn.setText(f"Show {rem} more errors...")
                else:
